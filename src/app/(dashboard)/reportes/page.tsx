...
export default async function ReportesPage({ searchParams }: { searchParams: Promise<{ anio?: string }> }) {
  ...
  const [kpis, resumenAnual, resumenAnterior] = await Promise.allSettled([
    getKpiEjecutivo(empresa.id),
    getResumenAnual(empresa.id, anio),
    getResumenAnual(empresa.id, anio - 1),
  ])

  const k = kpis.status === 'fulfilled' ? kpis.value : null
  const r = resumenAnual.status === 'fulfilled' ? resumenAnual.value : null
  const rAnt = resumenAnterior.status === 'fulfilled' ? resumenAnterior.value : undefined
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/reportes?anio=${anio - 1}`}
            className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-600 print:hidden">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {k.stock_bajo_minimo > 0 && (
              <Link href="/inventario/alertas" className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-4 hover:border-red-300">
          ...
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Ventas, compras, resultado y flujo de caja por mes
                {rAnt ? ` — comparado vs ${anio - 1}` : ''}.
              </p>
            </div>
          </div>
          <div className="p-5">
            <ResumenAnualTable resumen={r} resumenAnterior={rAnt} />
          </div>
        </div>
      )}
      ...
      <div className="print:hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Balance Comprobación', href: '/contabilidad/reportes/balance-comprobacion' },
            { label: 'Estado Resultados',    href: '/contabilidad/reportes/estado-resultados' },
            { label: 'Balance General',      href: '/contabilidad/reportes/balance-general' },
            { label: 'Declaración F29',      href: '/tributacion/f29' },
            { label: 'RLI / Renta',          href: '/tributacion/rli' },
            { label: 'Razones Financieras',  href: '/contabilidad/reportes/razones-financieras' },
            { label: 'Flujo de Caja',        href: '/finanzas/proyecciones' },
