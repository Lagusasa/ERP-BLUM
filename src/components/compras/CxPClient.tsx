...
export default function CxPClient({ documentos, empresa_id }: Props) {
  const router = useRouter()
  const [filtroProveedor, setFiltroProveedor] = useState('')
  const [procesando, setProcesando] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)

  // Solo documentos pendientes/contabilizados (no pagados ni anulados)
  const pendientes = useMemo(() =>
    documentos.filter((d) => d.estado !== 'pagado' && d.estado !== 'anulado'),
  [documentos])

  const proveedores = useMemo(() => {
    const set = new Map<string, string>()
    pendientes.forEach((d) => {
      if (d.proveedor_id && d.proveedor?.razon_social) set.set(d.proveedor_id, d.proveedor.razon_social)
    })
    return Array.from(set.entries()).sort((a, b) => a[1].localeCompare(b[1]))
  }, [pendientes])

  const filtrados = useMemo(() =>
    filtroProveedor ? pendientes.filter((d) => d.proveedor_id === filtroProveedor) : pendientes,
  [pendientes, filtroProveedor])
  ...
  const totalAlDia = filtrados.filter((d) => !d.fecha_vencimiento || new Date(d.fecha_vencimiento) >= hoy).reduce((s, d) => s + d.total, 0)

  async function marcarPagado(doc_id: string) {
    if (!confirm('¿Confirmar pago?')) return
    setProcesando(doc_id); setError(null)
    try {
      const res = await fetch('/api/compras/documentos', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: doc_id, accion: 'pagado' }),
      })
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          <p className="text-xs font-medium text-amber-600 uppercase tracking-wide">Por vencer (7 días)</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
          ...
          <span className="text-xs text-slate-400 ml-auto">
            Leyenda: <span className="text-amber-600 font-medium">0-30d</span> · <span className="text-orange-600 font-medium">31-60d</span> · <span className="text-red-600 font-medium">61-90d</span> · <span className="text-red-900 font-medium">+90d</span>
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtrados.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-10 text-slate-400">No hay documentos pendientes de pago.</td></tr>
              ) : (
                filtrados.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className={cn('hover:bg-slate-50 transition-colors', {
                      ...
                      <td className="px-5 py-2.5">
                        <p className="font-medium text-slate-800 text-sm">{d.proveedor?.razon_social ?? '—'}</p>
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <span className={cn('inline-block px-2 py-0.5 rounded-full text-xs font-medium', {
                          ...
                          {d.estado === 'pendiente' ? 'Pendiente' : 'Contabilizado'}
                        </span>
                      </td>
                      <td className="px-4 py-2.5 text-right">
                        <button onClick={() => marcarPagado(d.id)} disabled={procesando === d.id}
            ...
            {filtrados.length > 0 && (
              <tfoot className="border-t-2 border-slate-200 bg-slate-50">
                <tr>
                  <td colSpan={5} className="px-5 py-2.5 text-xs font-semibold text-slate-600 text-right">Total pendiente:</td>
