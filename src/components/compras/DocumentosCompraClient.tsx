...
function diasVencimiento(fechaVenc: string | null, estado: string): number | null {
  if (!fechaVenc || estado === 'pagado' || estado === 'anulado') return null
...
function VencimientoBadge({ dias }: { dias: number | null }) {
  ...
  if (dias < 0) {
    ...
    return (
      <span className={`inline-block text-xs px-1.5 py-0.5 rounded border ${color}`}>
        vence en {faltan}d
      </span>
    )
  }
...
export default function DocumentosCompraClient({ documentos, empresa_id }: Props) {
  ...
  async function contabilizar(doc_id: string) {
    ...
    try {
      const res = await fetch('/api/centralizacion/compra', {
        method: 'POST',
  ...
  async function marcarPagado(doc_id: string) {
    if (!confirm('¿Confirmar pago de este documento?')) return
    setProcesando(doc_id); setError(null)
    try {
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 flex-wrap">
          <div className="relative flex-1 min-w-48">
            ...
            <input type="text" placeholder="Buscar proveedor, N° documento..."
                    ...
                    <tr key={d.id} className={cn('hover:bg-slate-50 transition-colors', {
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          {(d.estado === 'contabilizado' || d.estado === 'pendiente') && (
