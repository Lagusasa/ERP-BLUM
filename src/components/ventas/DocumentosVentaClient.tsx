...
export default function DocumentosVentaClient({ documentos, empresa_id }: Props) {
  ...
  const exportarExcel = useCallback(() => {
    const rows = [
      ['Cliente', 'RUT', 'Tipo', 'N° Documento', 'Emisión', 'Vencimiento', 'Neto', 'IVA', 'Total', 'Estado'],
      ...filtrados.map((d) => [
        ...
        d.iva,
        d.total,
        ESTADO_VENTA_LABELS[d.estado] ?? d.estado,
      ]),
      [],
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Documentos Venta')
